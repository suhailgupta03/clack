...
/**
 * Initialize SocketServer action map
 * oto_message : one to one message
 * otm_message : one to many message
 */
SocketServer.prototype['oto_message'] = SocketServer.prototype.sendMessage;
    ...
    this.socketServer.on("connection",function(socket) {
        ...
        socket.on("message",function(message) {
            ...
            try {
                ...
                if(o.action && SocketServer.prototype[o.action]) {
                    // Cater to the request
                    that.sendMessage(socket.userAttached,socket.to,socket.message);
                }
            }catch(e){
                throw e;
            }
...
/**
 * Map the request string to the appropriate function
 */
SocketServer.prototype.mapTo = function() {
...
SocketServer.prototype.sendMessage = function(senderID, receiverID, message) {
    ...
    this.socketServer.forEach(function(socket) {
        if(socket.userAttached && socket.userAttached == userID) {
            receiverFound = true;
            socket.send(message);
        }
    });

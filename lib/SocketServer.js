...
SocketServer.prototype.broadcast = function(message, groupName) {
    ...
    if(!groupName) {
        ...
        if((this.couchePipe instanceof CouchePipe)) {
            ...
            var groupExists = (chunk,response) => {
                // If the group exists, get the users attached
                var groupList = JSON.parse(chunk);
                if(Array.isArray(groupList.usersAttached)) {
                    var groupUsers = groupList.usersAttached;
                    // Get sockets for all the users inside the group
                    var filteredSockets = this.socketServer.clients.filter(function(client) {
                       return groupUsers.indexOf(client.userAttached) != -1; 
                    });
                    if(filteredSockets.length > 0) {
                        filteredSockets.forEach(function(client) {
                            client.send(`~Broadcast: ${message}`); 
                        });   
                    }
                }            
            }
            this.couchePipe.on("dfetched",groupExists);
            this.couchePipe.on("derror",function(chunk,response) {
